...
- CrossIntersection 방지 로직 적용
  - 주제 : 미션의 요구사항으로는 존재하지 않지만, 사다리게임에서 중요한 요소인 CrossIntersection 상황을 방지하기 위해 노력했습니다.
  - 왜냐하면 : 명시적으로 제한사항이 제시되지는 않았지만 사다리게임의 필수요소라고 생각하기 때문입니다.
  - CrossIntersection 이란 사다리게임에서 같은 높이에 사다리 라인(-----) 이 두번 연속 등장하는 경우를 의미합니다.
