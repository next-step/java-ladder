...
- [ ] 사다리 게임에 참여하는 사람의 이름은 쉼표(,)를 기준으로 구분한다.
- [ ] 사다리 게임은 참여하는 사람의 이름과 최대 사다리 높이를 전달받는다.
- [ ] 사다리 타기는 (참여하는 사람의 수 - 1) 만큼 라인이 생성된다.
- [ ] 사다리 타기는 라인이 겹치지 않는다.
- [ ] 사다리의 폭은 최대 글자 수 만큼으로 출력한다.
- [ ] 사다리는 랜덤값 등 사다리 생성 전략 따라서 생성한다.
