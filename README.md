...
- [ ] 쉼표(,)로 구분된 문자열로부터 참여자 리스트를 생성한다
  - [ ] 이름 리스트를 사다리 상단에 출력 형식에 맞게 표시한다
  - [ ] 이름이 5자 기준으로 출력되도록 포맷팅한다
  - [ ] 사다리 폭은 이름 길이에 따라 조정된다
- [ ] 사다리를 구성할 라인을 생성한다
  - [ ] 라인은 수평 연결 정보를 가진다 (왼쪽에서 오른쪽으로 이동 가능한지)
  - [ ] 라인은 임의의 위치에 수평선을 추가할 수 있다
  - [ ] 인접한 수평 라인이 겹치지 않도록 한다
  - [ ] 겹치는 라인이 있는 경우 예외를 던진다 또는 무시한다
