...
- 1.1. 각 플레이어는 각 위치에서 왼쪽, 오른쪽 라인이 존재하는지 판단한다.
  - 각 숫자는 위치(Index)를 의미한다.
  - Left Line Index = Player Index -1
  - Right Line Index = Player Index
...
- 1.2. 라인(`Line`)이 존재(`true`)는 위치(`Index`)를 다음 깊이(`Depth`)의 시작 라인으로한다.
  - 오른쪽 라인이 존재하면 Player Index = Player Index++
  - 왼쪽 라인이 존재하면 Player Index = Player Index--
  - 존재하지 않는 라인이면 Index 유지
- 1.3. 마지막 Depth 까지 위치를 저장하며 반복한다.
- 1.4. 플레이어의 마지막 위치를 담고 있는 결과 데이터를 비교하여 출력한다.
