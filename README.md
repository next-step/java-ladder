...
`요구사항 및 구현 TODO 리스트 `
- [ ] 입력 UI
   - [ ] 사다리 게임 참여 사람 이름 입력 UI (최대5글자)
   - [ ] 최대 사다리 높이 입력 UI
- [ ] 실행 결과 출력
   - [ ] 참여자 이름 출력(쉼표 기준 구분)
   - [ ] 사다리 출력
   - [ ] 사다리 폭은 사람이름 5글자 고려
   - [ ] 사다리 세로는 이름의 중앙에 위치하도록 구현
- [ ] 도메인 기능
   - [ ] 입력 UI 에서 전달한 사람이름 문자열 분리(쉼표로 구분)
   - [ ] 참여자들의 이름 상태를 가지는 객체 구현
     - [ ] 유효한 이름 문자열 구분자 포함 여부 확인 
     - [ ] 참여자 수와 사다리의 가로 넓이가 같은지 확인 
   - [ ] 이름 객체 구현
     - [ ] 이름 길이 유효성검증 : 5글자 제한
   - [ ] 사다리의 한줄의 상태를 가지는 라인 객체 구현
     - [ ] 사다리의 수평선의 유무는 boolean 으로 표현
     - [ ] 사다리의 넓이는 양수일것
   - [ ] 여러줄의 상태를 가지는 사다리 객체 구현
     - [ ] 일급컬렉션으로 구현
     - [ ] 사다리의 높이는 양수일것
     - [ ] 사다리의 넓이와 참가자수는 같은지 확인
   - [ ] 사다리 수평선 생성 전략
     - [ ] 랜덤 전략
       - [ ] 사다리의 첫번째 수평선은 랜덤하게 생성
       - [ ] 연속해서 수평선이 생성되지 않는 규칙
       - [ ] 이전 수평선이 없는 경우는 랜덤 생성
     - [ ] 고정 전략
       - [ ] 테스트를 위한 고정 생성 전략
   - [ ] 사다리 게임 객체
     - [ ] 참여자와 사다리의 상태를 가지는 사다리 게임 객체
     - [ ] 사다리가 올바르게 생성되었는지 확인
       - [ ] 사다리의 넓이와 참여인원수는 같아야 한다
---
