...
class LadderTest {
    ...
    @Test
    void LadderTest() {
        assertThat(new Ladder(3, new String[]{"꽝", "당첨", "꽝"}, 3)).isInstanceOf(Ladder.class);
    }

    @DisplayName("참가자 수와 결과 배열의 수가 일치하지 않으면 IllegalArgumentException을 던진다.")
    @Test
    void ladderExceptionTest() {
        assertThatThrownBy(() ->new Ladder(3, new String[]{"꽝", "당첨"}, 3))
    ...
    void LadderResultTest() {
        Ladder ladder = new Ladder(3, new String[]{"꽝", "당첨", "꽝"}, 3);
