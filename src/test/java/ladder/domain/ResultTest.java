...
class ResultTest {
    ...
    void constructor(List<String> result) {
        assertThat(new Result(result.size(),result)).isNotNull();
    }
    ...
    void constructor_when_invalid_result(int countOfPerson, List<String> result) {
        assertThatThrownBy(() -> new Result(countOfPerson, result)).isInstanceOf(IllegalArgumentException.class)
                                                                   .hasMessage("The number of results must be greater than 1 and equal to the number of users.");
    }
