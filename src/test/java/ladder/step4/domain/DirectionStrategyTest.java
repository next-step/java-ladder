...
public class DirectionStrategyTest {
    ...
    private static final DirectionStrategy strategy = BodyDirectionStrategy.getInstance();

    @DisplayName("사다리에 줄이 생기지 말아야할 경우에 대한 테스트")
    @Test
    void 사다리_줄_생성_테스트() {
        assertEquals(false, strategy.create(true));
    }

    @DisplayName("새로운 전략 생성에 대한 테스트")
    @ParameterizedTest
    @MethodSource("provideLadderStrategy")
    void 사다리_줄_전략_테스트(DirectionStrategy strategy, boolean prev, boolean expected) {
        assertEquals(expected, strategy.create(prev));
    }

    private static Stream<Arguments> provideLadderStrategy() {
        DirectionStrategy alwaysTrueStrategy = prev -> true;
