...
class LineFactoryTest {
    ...
    void create(int countOfPerson) {
        Line line = LineFactory.create(countOfPerson, new RandomPointPickerStrategy());
    }

    @ParameterizedTest(name = "부족한 인원의 사람 수가 주어질 때, 예외를 반환합니다.")
    @ValueSource(ints = {0, 1})
    void createWithInsufficientPeopleCount(int countOfPerson) {
        assertThatThrownBy(() -> LineFactory.create(countOfPerson, new RandomPointPickerStrategy()))
    ...
    void hasValidMovingPoints(int countOfPerson, List<Boolean> expectedMovingPoints) {
        List<Boolean> movingPoints = LineFactory.create(countOfPerson, () -> true).getMovingPoints();
