...
public class LineTest {
    ...
    @DisplayName("사다리 발판 생성 테스트 :: 참여자 수를 입력하면 List<MountingBlock>이 몇개 생성되는지?")
    @ParameterizedTest
    @MethodSource("provideCountOfPerson")
    void mountingBlocksCountTest(final Integer countOfPerson, final Integer expected) {
        ...
        assertThat(mountingBlocks.size()).isEqualTo(expected);
    }

    private static Stream<Arguments> provideCountOfPerson() {
        return Stream.of(
                Arguments.of(4, 3),
                Arguments.of(5, 4),
                Arguments.of(10, 9)
    ...
    void lineConstructorTest(final Integer countOfPerson, final Queue<Boolean> wanted, final List<Boolean> expected) {
        ...
        Line line = new Line(countOfPerson, new TestQueueMountingBlockGenerator(wanted));
        List<MountingBlock> mountingBlocks = line.getMountingBlocks();
