...
public class RungTest {
    ...
    void generate(Direction originDirection, Direction expectedDirection) {
        assertThat(new Rung(originDirection).generate(() -> true))
                .isEqualTo(new Rung(expectedDirection));
    }

    @DisplayName("rightConnected는 발판이 우측으로 연결되었는지 여부를 반환한다.")
    @Test
    void rightConnected() {
        assertThat(new Rung(Direction.RIGHT).rightConnected())
                .isTrue();
    }
    ...
    @Test
    void leftConnected() {
        assertThat(new Rung(Direction.LEFT).leftConnected())
                .isTrue();
    }
    ...
    @Test
    void notConnected() {
        assertThat(new Rung(Direction.NONE).notConnected())
                .isTrue();
    }
}
