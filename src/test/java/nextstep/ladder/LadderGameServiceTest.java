...
public class LadderGameServiceTest {
    private LadderGameService service;
    ...
    public void setUp() {
        ...
        ladder = new Ladder(4, List.of(new Line(true, false, true),
    ...
    void play_사다리게임을_실행하면_게임결과를_반환한다() {
        LadderGameResult gameResult = service.play(players, prizes, ladder);

        Iterator<Integer> expectedPrizes = List.of(0, 3, 2, 1)
                .iterator();
        players.forEach(player -> assertThat(gameResult.getValue()).containsEntry(player,
                                                                                  prizes.get(expectedPrizes.next())
        ));
    }
}
