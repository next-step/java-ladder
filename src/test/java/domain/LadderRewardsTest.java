...
public class LadderRewardsTest {
    ...
    public void 결과를_입력받는다() {
        new LadderReward("꽝,담첨,꽝,꽝", new Players("홍길동,홍길순,나인제로,둘리"));
    }

    @Test(expected = IllegalArgumentException.class)
    public void 결과값이_NULL이면_예외를_던진다() {
        new LadderReward(null, new Players("홍길동,홍길순,나인제로,둘리"));
    }

    @Test(expected = IllegalArgumentException.class)
    public void 결과값이_빈값이면_예외를_던진다() {
        new LadderReward("", new Players("홍길동,홍길순,나인제로,둘리"));
    }

    @Test
    public void 결과값의_길이를_콤마를_구분자로_나눈다() {
        final LadderReward ladderReward = new LadderReward("꽝,담첨,꽝,꽝", new Players("홍길동,홍길순,나인제로,둘리"));
    ...
    public void 결과값의_숫자가_참가자_숫자와_다르면_예외를_던진다() {
        new LadderReward("꽝,담첨,꽝,꽝", new Players("홍길동,홍길순,나인제로"));
    }

    @Test
    public void 참가자_번호로_어떤_보상을_받을수_있는지_검사() {

        final LadderReward ladderReward = new LadderReward("꽝1,당첨,꽝2,꽝3", new Players("홍길동,홍길순,나인제로,둘리"));
