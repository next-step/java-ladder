...
import step4.exceptions.OutOfRangePlayerNameException;
...
public class ConsoleViewImpl implements View {
    ...
    public static final String ERROR_OUT_OF_RANGE_RESULTS = "실행결과는 %d개여야 합니다.";
    ...
    private void isValidResultSize(List<String> strings, int maxSize) {
        if (strings.size() != maxSize) {
            throw new IllegalArgumentException(String.format(ERROR_OUT_OF_RANGE_RESULTS, maxSize));
        }
    }
    ...
    public void drawPlayResult(PlayerWinningInfo playerWinningInfo) {
        try {
            resultView.drawPlayResult(playerWinningInfo);
        } catch (Exception error) {
            System.out.println(error.getMessage());
        }
    }
    ...
    public void drawPlayResultAll(LadderGameResults playResults) {
        try {
            resultView.drawPlayResultAll(playResults);
        } catch (Exception error) {
            System.out.println(error.getMessage());
        }
    }
