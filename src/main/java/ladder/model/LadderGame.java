...
public class LadderGame {
    ...
    public List<Boolean> makeLine(LadderLineStrategy ladderLineStrategy) {
        ...
        for (int userCount = 0; userCount < participant.getUserSize(); userCount = pointList.size()) {
            pointList.add(ladderLineStrategy.boolLine());
            setPointList(pointList, this.participant.getUserSize() - 1);
        }
        return pointList;
    }

    private void setPointList(List<Boolean> pointList, int userCount) {
